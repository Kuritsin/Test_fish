...
def find_candidates(mask: np.ndarray, config: BotConfig,
    ...
    profiled_maximum = max(config.auto_find_max_area, profile.max_area) if profile else config.auto_find_max_area
    maximum = min(config.auto_find_hard_max_area, profiled_maximum)
    ...
    for group in _component_groups(components, config.auto_component_merge_gap):
        ...
        x = int(round(sum(item[5] * item[4] for item in group) / area))
        y = int(round(sum(item[6] * item[4] for item in group) / area))
        ...
        features = score_candidate(frame, (left, top, width, height), config.auto_candidate_padding,
                                   templates, config.auto_template_scales, config.auto_template_weight) \
...
def _component_groups(
    components: list[tuple[int, int, int, int, int, float, float]], gap: int,
) -> list[list[tuple[int, int, int, int, int, float, float]]]:
    groups: list[list[tuple[int, int, int, int, int, float, float]]] = []
    for component in components:
        touching = [group for group in groups if any(_boxes_near(component, other, gap) for other in group)]
        if not touching:
...
def _boxes_near(first, second, gap: int) -> bool:
    first_right, first_bottom = first[0] + first[2], first[1] + first[3]
    second_right, second_bottom = second[0] + second[2], second[1] + second[3]
    horizontal = max(0, first[0] - second_right, second[0] - first_right)
    vertical = max(0, first[1] - second_bottom, second[1] - first_bottom)
...
class AutomaticBobberFinder:
    ...
    def find(
        ...
        while time.monotonic() < deadline and not stop_event.is_set() and safe():
            ...
            raw_candidates = find_candidates(mask, self.config, self.profile, frame, self.templates)
            if raw_candidates:
                best_score = max(best_score, raw_candidates[0].score)
            candidates = raw_candidates if permissive else [
                item for item in raw_candidates if item.score >= self.config.auto_min_candidate_score
            ]
            if not permissive and self.templates:
                template_candidates = [item for item in candidates if item.features is not None
                                       and item.features.template >= self.config.auto_min_template_score]
                if candidates and not template_candidates:
                    template_rejected_seen = True
                candidates = template_candidates
            separated = (len(candidates) > 1 and
                         ((candidates[0].x - candidates[1].x) ** 2
                          + (candidates[0].y - candidates[1].y) ** 2) ** 0.5
                         > self.config.auto_find_match_radius)
            if (not permissive and separated
                    and candidates[0].score - candidates[1].score < self.config.auto_min_score_gap):
                ambiguous_seen = True
                candidates = []
            ...
            if debug_dir is not None:
                self._debug(frame, mask, raw_candidates, debug_dir)
            if previous is not None and confirmations >= self.config.auto_find_confirmation_frames:
                ...
                return AutomaticBobberResult(detection, template, "novel_colour_component", previous.score)
            if stop_event.wait(self.config.bobber_search_interval):
                break
        if template_rejected_seen:
            reason = "template_candidates_rejected"
        elif ambiguous_seen:
            reason = "ambiguous_candidates"
        else:
            reason = "automatic_search_timeout"
        return AutomaticBobberResult(BobberDetection(False), reason=reason, best_score=best_score)
