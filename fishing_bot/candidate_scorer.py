...
def score_candidate(frame: np.ndarray, box: tuple[int, int, int, int], padding: int = 12,
                    ...
                    scales: tuple[float, ...] = (1.0,), template_weight: float = 0.65) -> CandidateFeatures:
    ...
    red = ((hue <= 18) | (hue >= 162)) & (saturation >= 70) & (value >= 45)
    blue = (hue >= 90) & (hue <= 138) & (saturation >= 70) & (value >= 45)
    ...
    red_score = min(1.0, float(np.count_nonzero(red)) / max(2.0, pixel_count * 0.012))
    blue_score = min(1.0, float(np.count_nonzero(blue)) / max(2.0, pixel_count * 0.02))
    colour_score = 0.65 * red_score + 0.35 * blue_score
    body_score = min(1.0, float(np.count_nonzero(body)) / max(5.0, pixel_count * 0.14))
    ...
    strip_width = max(3, width // 2)
    center = min(roi.shape[1] - 1, max(0, left + width // 2 - x0))
    sx0, sx1 = max(0, center - strip_width), min(roi.shape[1], center + strip_width + 1)
    strip_edges = edges[:, sx0:sx1]
    lines = cv2.HoughLinesP(strip_edges, 1, np.pi / 180, threshold=7,
                            minLineLength=max(6, height), maxLineGap=4)
    line_score = 0.0
    if lines is not None:
        for x_start, y_start, x_end, y_end in np.asarray(lines).reshape(-1, 4):
            dx, dy = abs(int(x_end) - int(x_start)), abs(int(y_end) - int(y_start))
            if dy >= max(6, height) and dx <= max(2, dy * 0.25):
                line_score = max(line_score, min(1.0, dy / max(8.0, height * 2.0)))
    ...
    weight = min(0.9, max(0.0, template_weight))
    total = base if not templates else (1.0 - weight) * base + weight * template_score
